...
# Lidar2 is 7.366 meters East of Lidar1
...
# --- Configuration ---
VERSION = "1.056 2026-08-18"  # add daily events-summary CSV, same style as read_sen0648.py
OUTPUT_DIR    = '/dev/shm/L1'

# --- Daily events-summary CSV (one row per event, rotated at local midnight) ---
# Same directory/lifecycle as per-event CSVs: written to tmpfs, then pushed to
# REMOTE_DEST and deleted locally once the day's file is finalized. Column
# layout is based on read_sen0648.py's sen0648_events_*.csv, minus the
# columns that have no equivalent here at all (TF02-Pro has no onboard
# sample clock or exposed checksum, and lidar_events2.py doesn't track a
# signal baseline or a spurious-event flag: start_ms, crc_fail, base_signal,
# base_signal_sd, spurious are omitted rather than always written as "n/a").
# A few extra columns lidar_events2.py can uniquely provide (category,
# range_rms_mm, rise_ratio, str_avg, str_rms) are appended after the common
# columns.
DAILY_SUMMARY_DIR = OUTPUT_DIR
DAILY_CSV_FIELDS = [
    "start_epoch", "start_local",
    "duration_ms", "median_dist_m", "min_dist_m", "max_signal", "status0",
    "base_dist_m", "base_dist_sd_m",
    "category", "range_rms_mm", "rise_ratio", "str_avg", "str_rms",
]
_daily_csv_file = None
_daily_csv_writer = None
_daily_csv_date = None   # local date string (%Y%m%d) of the currently-open file
_daily_csv_path = None   # path of the currently-open (still being appended to) file
_daily_csv_lock = threading.Lock()

# A single wall-clock-aligned thread (ticking at local :00 each hour) does
# double duty:
#  - at the 00:00 tick: finalizes the day's file (close it, hand it to the
#    existing _transfer_queue for push-then-delete-locally, same as
#    per-event CSVs) and opens a fresh file for the new day. This replaces
#    the old "rotate on first event of the new day" logic.
#  - at every other hourly tick: pushes the still-open, still-growing file
#    as a plain overwrite copy (no removal), so a ramdisk power loss loses
#    at most ~1 hour of the current day. Same filename each time, so rsync
#    just overwrites the previous copy on the receiving end.
...
def _daily_finalize_and_rotate():
    """Close the current daily file, queue it for push+removal, open a new one."""
    global _daily_csv_file, _daily_csv_writer, _daily_csv_date, _daily_csv_path
    with _daily_csv_lock:
        old_path = None
        if _daily_csv_file is not None:
            old_path = _daily_csv_file.name
            _daily_csv_file.close()

        day_str = time.strftime("%Y%m%d")  # local date, now that midnight has passed
        fname = os.path.join(DAILY_SUMMARY_DIR, f"lidar1_daily_{day_str}.csv")
        _daily_csv_file = open(fname, "a", newline="")
        _daily_csv_writer = csv.DictWriter(_daily_csv_file, fieldnames=DAILY_CSV_FIELDS)
        if _daily_csv_file.tell() == 0:
            _daily_csv_writer.writeheader()
            _daily_csv_file.flush()
        _daily_csv_date = day_str
        _daily_csv_path = fname
...
def _daily_hourly_push_loop():
    while True:
        time.sleep(_seconds_until_next_local_hour())

        if time.localtime().tm_hour == 0:
            _daily_finalize_and_rotate()
            continue

        with _daily_csv_lock:
            path = _daily_csv_path
            if path is not None and _daily_csv_file is not None:
                _daily_csv_file.flush()
        ...
        try:
            ...
            if result.returncode != 0:
                print(f"[daily push] rsync failed for {path}: {result.stderr.strip()}")
            else:
                print(f"[daily push] pushed {path} -> {REMOTE_DEST} (local copy kept)")
        except Exception as e:
            print(f"[daily push] error: {e}")


def _daily_local_str(epoch):
    return time.strftime("%H:%M:%S", time.localtime(epoch)) + f".{int(epoch % 1 * 1000):03d}"


def _write_daily_summary_row(row: dict):
    """
    Append one row to today's daily events-summary CSV. Day rollover (close,
    push+remove, open new file) is handled solely by the midnight tick of
    _daily_hourly_push_loop; this only lazily opens the very first file if
    the sensor loop starts up with none open yet.
    """
    global _daily_csv_file, _daily_csv_writer, _daily_csv_date, _daily_csv_path

    with _daily_csv_lock:
        if _daily_csv_file is None:
            day_str = time.strftime("%Y%m%d", time.localtime(row["start_epoch"]))
            fname = os.path.join(DAILY_SUMMARY_DIR, f"lidar1_daily_{day_str}.csv")
            _daily_csv_file = open(fname, "a", newline="")
            _daily_csv_writer = csv.DictWriter(_daily_csv_file, fieldnames=DAILY_CSV_FIELDS)
            if _daily_csv_file.tell() == 0:
            ...
            _daily_csv_date = day_str
            _daily_csv_path = fname
            print(f"[daily] now logging to {fname}")

        _daily_csv_writer.writerow(row)
        _daily_csv_file.flush()

# Remote host/path events are pushed to after being written locally.
# Transfers run one at a time in a background thread (queued, not overlapping).
REMOTE_DEST   = "jbeale@jbeale-mini.local:/mnt/bluecherry/LIDAR1/"

# Camera notify: send LiDAR event summary to vehicle_detect.py on the camera machine.
# Set CAMERA_HOST = None to disable.
CAMERA_HOST        = "192.168.1.164"   # rp64.local "192.168.1.164"
CAMERA_PORT        = 5006              # UDP port vehicle_detect.py listens on (L1: 5006, L2: 5007)
...
def _probe_tf02_port(port, read_timeout=1.0, required_valid_frames=3):
    """
    Briefly open `port` at the TF02-Pro's baud rate and check whether it's
    actually emitting valid TF02-Pro frames (0x59 0x59 header, correct
    ...
    try:
        ...
        buf = bytearray()
        valid_frames = 0
        while time.time() < deadline and valid_frames < required_valid_frames:
            chunk = probe.read(64)
            if not chunk:
                continue
            buf.extend(chunk)
            while len(buf) >= 9:
                ...
                if (sum(frame[:8]) & 0xFF) == frame[8]:
                    ...
                    buf.pop(0)
        return valid_frames >= required_valid_frames
    finally:
        probe.close()
        time.sleep(0.2)  # let the adapter settle before it gets reopened for real


def find_tf02_port():
    """
    Return the serial port path for the CH341 adapter that's actually
    talking the TF02-Pro protocol, out of possibly several identically
    VID:PID'd (1a86:7523) CH341 adapters on this machine. Each candidate
    is probed for real TF02-Pro frames; a candidate that's already open
    elsewhere (busy) is skipped rather than fought over.
    """
    candidates = [
        p.device
    ...
    if not candidates:
        raise RuntimeError("No CH341 USB-serial adapter found")

    for port in candidates:
        print(f"Probing {port} for TF02-Pro frames...")
        if _probe_tf02_port(port):
            print(f"  -> confirmed TF02-Pro on {port}")
            return port
        print(f"  -> not TF02-Pro on {port} (busy, or a different device)")

    raise RuntimeError(
        f"No CH341 adapter is emitting valid TF02-Pro frames "
        f"(checked: {', '.join(candidates)}). Is the TF02-Pro connected/powered, "
        f"and is its adapter free (not already held open by another process)?"
    )
...
_web_state  = {                        # latest single-sample snapshot
    "dist": 0, "dist_avg": 0, "strength": 0, "temp_c": 0.0, "pi_temp_c": 0.0,
    "bg_mean": 0.0, "bg_std": 0.0, "trigger": 0.0,
    "in_event": False, "category": "", "ts": 0.0,
    "last_event_id": 0.0,              # epoch of most recent completed event
}

PI_TEMP_POLL_SECS = 10   # how often to read the Pi CPU temperature

def _read_pi_temp_c():
    """Return Raspberry Pi CPU temperature in °C via vcgencmd, or None on failure."""
    try:
...
def _pi_temp_loop():
    while True:
        ...
        if t is not None:
            with _web_lock:
                _web_state['pi_temp_c'] = t
        time.sleep(PI_TEMP_POLL_SECS)

# ---------------------------------------------------------------------------
# Remote push: event CSVs land in OUTPUT_DIR (/dev/shm/L2), then get queued
# here for transfer to REMOTE_DEST. A single worker thread processes the
# queue serially, so transfers never overlap. rsync --remove-source-files
# deletes the local copy automatically once the transfer succeeds; on
# failure the file is left in place and retried on the next queue pass.
# ---------------------------------------------------------------------------
...
<div id="meta">
  ...
  <span>temp L: <b id="m-temp">–</b> °C</span>
  <span>temp Pi: <b id="m-temp-pi">–</b> °C</span>
</div>
...
src.onmessage = e => {
  ...
  document.getElementById('m-temp-pi').textContent = msg.pi_temp_c.toFixed(1);
...
def _sensor_loop():
    ...
    while True:   # ── outer restart loop ──────────────────────────────────────
        ...
        # Continuously-blocked suppression: if the beam stays blocked across
        # two full MAX_EVENT_SECS timeouts, stop generating new events until
        # the signal returns to near background.
        continuously_blocked   = False   # suppression active flag
        blocked_interval_count = 0       # consecutive timeout intervals so far
        blocked_rearm_count    = 0       # samples near background while suppressed
        ...
        while True:
            ...
            if not in_event:
                ...
                # While beam is continuously blocked, wait for signal to recover
                # before allowing new triggers.
                if continuously_blocked:
                    if abs(d - bg_mean) <= REARM_DIFFERENCE:
                        blocked_rearm_count += 1
                    else:
                        blocked_rearm_count = 0
                    if blocked_rearm_count >= REARM_SAMPLES:
                        continuously_blocked   = False
                        blocked_interval_count = 0
                        blocked_rearm_count    = 0
                        below_count            = 0
                        print(f"[{time.strftime('%H:%M:%S')}] continuously-blocked cleared — beam returned to background")
                    # Never trigger while suppressed; skip to status check below.

                # Check trigger: N consecutive samples below threshold
                elif d < trigger_thresh:
                    below_count += 1
                else:
                    below_count = 0

                if not continuously_blocked and below_count >= TRIGGER_N:
                    in_event = True
                    rearm_count = 0
                    event_samples = []
                ...
                if rearm_count >= REARM_SAMPLES:
                    end_reason = "rearm"
                    blocked_interval_count = 0   # clean rearm — reset blocked counter
                elif len(event_samples) >= MAX_EVENT_SAMPLES:
                    end_reason = "timeout"
                    blocked_interval_count += 1
                    if blocked_interval_count >= 2:
                        continuously_blocked = True
                        blocked_rearm_count  = 0
                        print(f"[{time.strftime('%H:%M:%S')}] beam continuously blocked "
                              f"({blocked_interval_count} intervals) — suppressing new events")
                ...
                if end_reason:
                    ...
                    if rec and result:
                        # off-baseline, non-dropout samples -- same set used for str_avg/str_rms above
                        valid_dists_mm = [s[1] for s in event_samples if 0 < s[1] < 45000]
                        dropout_count  = len(event_samples) - len(valid_dists_mm)
                        median_dist_m  = round(np.median(valid_dists_mm) / 1000.0, 3) if valid_dists_mm else None
                        min_dist_m     = round(min(valid_dists_mm) / 1000.0, 3) if valid_dists_mm else None
                        max_signal     = max(strengths) if strengths else None

                        # analyze_event's 50%-excursion window, offset from event_start_epoch
                        # (index 0 of event_samples) -- this is the closest equivalent to
                        # read_sen0648.py's off-baseline EVENT START. End time isn't stored;
                        # it's start_epoch + duration_ms/1000 if ever needed.
                        win_start_epoch = event_start_epoch + result['dur_start_ms'] / 1000.0

                        _write_daily_summary_row({
                            "start_epoch":    round(win_start_epoch, 3),
                            "start_local":    _daily_local_str(win_start_epoch),
                            "duration_ms":    result['duration_ms'],
                            "median_dist_m":  median_dist_m if median_dist_m is not None else "n/a",
                            "min_dist_m":     min_dist_m if min_dist_m is not None else "n/a",
                            "max_signal":     max_signal if max_signal is not None else "n/a",
                            "status0":        dropout_count,
                            "base_dist_m":    round(bg_mean / 1000.0, 4),
                            "base_dist_sd_m": round(bg_std / 1000.0, 4),
                            "category":       result['category'],
                            "range_rms_mm":   result['range_rms_mm'],
...
threading.Thread(target=_pi_temp_loop, daemon=True, name='pi_temp').start()
threading.Thread(target=_transfer_worker, daemon=True, name='transfer').start()
threading.Thread(target=_daily_hourly_push_loop, daemon=True, name='daily_push_rotate').start()
